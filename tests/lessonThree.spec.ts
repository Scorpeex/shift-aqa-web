...
test('Smoke FAQ form test', async ({ page, request }) => {
...
test('Smoke FAQ form test with mock - wrong but valid captcha', async ({ page, request }) => {
...
test('Проверка 500 ответа', async ({ page, request }) => {
  await page.route('api/feedback', async (route) => {
    await route.fulfill({
        status: 500,
        contentType: 'application/json',
...
test('Негативная прокерка капчи - меньше минимальной длины', async ({ page, request }) => {
  ...
  await fillAllFields(page, {code: 'qqq'})
  ...
  await expect(page.getByTestId('modal-message')).toBeHidden()
});

test('Тест сортировки по возрастанию текущей страницы', async ({ page, request }) => {
...
test('Тест сортировки по возрастанию все страницы', async ({ page, request }) => {
    await page.goto('catalog')
    await page.getByTestId('catalog-sort-select').click()
    await page.getByTestId('catalog-sort-option-asc').click()
